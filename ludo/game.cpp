...
{
    if(debug) std::cout << "Win cnt init\n";
...
{
    if(debug) std::cout << "Start with init!" << std::endl;
    if(!firstInit)
    {
        if(debug) std::cout << "first init" << std::endl;
        ...
        QObject::disconnect(this,SIGNAL (declare_winner(int)),              &(*pp1), SLOT(gameCounter(int)));
    ...
    if(debug) std::cout << "Begin to connect" << std::endl;
    ...
    QObject::connect(this,SIGNAL (declare_winner(int)),              &(*pp1), SLOT(gameCounter(int)));
    ...
    if(debug) std::cout << "connection established\n";

    if(debug) std::cout << "Init end" << std::endl;
    if(firstInit == true)
        firstInit = false;
    gameTrainFlag = false;
    gameFlag = true;
...
void game::add_players(ludo_evo_player *p1, ludo_player_random *p2, ludo_player_random *p3, ludo_player_random *p4)
{
    pp1 = p1;
    pp2 = p2;
    pp3 = p3;
...
void game::turnComplete(bool win){
    ...
    turn_complete = true;
    if(game_complete){
        if(debug) std::cout << "player: " << color << " won" << std::endl;
        switch(color){
        case 0:
            winCnt1++;
            break;
